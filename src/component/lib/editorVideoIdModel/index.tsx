...
export default class extends React.Component<Props, State> {
  ...
  // 父组件 调用子组件的地方
  _showVideoIdVisible = (cb:Function) =>{
    // status 控制弹窗的状态
    // cb 是回调函数，弹窗确认时还要传回去
    // cb回传的格式      cb({value:'外层视频'})
  ...
  _videoIdOk = () => {
    ...
    // this.videoIdCB 时父组件传过来的函数，控制富文本绘制用的
    // cb回传的格式      cb({value:'外层视频'})
    this.props.drawVideoIdEditor(this.videoIdCB,{
      link_url: videoIdValue,
      content:videoIdTagValue,
    })
  }
  ...
  _renderModel = () => {
    ...
    >
      <Row align="middle" justify="center" type="flex">
        ...
        <Col span={18} >
          <Input value={this.state.videoIdValue} placeholder="请输入腾讯视频ID" onChange={this._changeVideoIdValue} />
        </Col>
      </Row>
